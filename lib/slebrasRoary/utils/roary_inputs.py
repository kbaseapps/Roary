...
def download_gffs(cb_url, scratch, genome_set_ref):
	...
	for ref in refs:
		...
		if gen_obj['domain'] not in ['Bacteria','Archaea']:
			raise TypeError('Provided Genomes are not labeled as Bacteria or Archaea. Roary is only equipped to handle Archaea or Bacteria')
		...
		if contains_fasta:
			...
			args = ['cat',fasta_file['path'], cat_path, gff_file_path, '>', new_file_path]
			subprocess.call(args)
